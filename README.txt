...
* Cater for missing ratio being passed through
* Update URLs in plugin description and update file to reference https
* PHP 8.2 compatibility
* Declare support for WordPress 6.3.2

= 1.0.54 =
* New filter so image ratio can be customised
* Change template assistant custom field Elementor widget icon
* If type of additional field is changed, change it in any search forms too
* Round top positions when trying to make all rows the same height
* Declare support for WordPress 6.3

= 1.0.53 =
* Added min/max reception rooms as fields available to add to search forms
* Added additional field type of 'Checkbox'
* Declare support for WordPress 6.1.1
...
* Add span around custom fields labels output on frontend so they can be hidden
...
= 1.0.47 =
* Added support for new additional field type of 'File'

= 1.0.46 =
* Added currency field to search forms allowing user to specify currency shown throughout site
* Corrections to maximum bedrooms field including default options and control class name
...
* Declare support for WordPress 5.7.2

= 1.0.43 =
* Corrected issue with Generate Applicant List always returning true when checking custom fields and thus returning wrong applicants
* Declare support for WordPress 5.7.1

= 1.0.42 =
* Add setting to specify image size used in search results
* Allow custom fields to be added to deposit scheme and meter readings meta boxes
* Only show management additional field departments in section dropdown when lettings department is active
* Declare support for WordPress 5.7

= 1.0.41 =
* Add ability to add custom fields to new tenancy and management meta boxes
* Add support for custom fields being used after recent change showing all profiles within user My Account area
...
= 1.0.39 =
* Add 'Keyword' as a new field when building search forms following recent support for it being added to core plugin
* Add any additional fields specified as being used on property matches to the 'Generate Applicant List' filter
* Call wp_enqueue_media() when relevant if office image field being used. For some reason on some installations the media JS wasn't being included
...
* Added ability to add additional fields to offices
* Any additional fields added can now be included in documents when using our Documents add on
